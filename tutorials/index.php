<?php include $_SERVER["DOCUMENT_ROOT"].'/includes/pre_main.php'; ?>

<h1> TUTORIALS </h1>
<ul>
    <li>Linux</li>
    <ul>
      <li><a href="/tutorials/linux/disks.php">Disks</a></li>
      <li><a href="/tutorials/linux/check_success.php">Check command success</a></li>
      <li><a href="/tutorials/linux/lamp.php">LAMP</a></li>
      <li><a href="/tutorials/linux/nextcloud.php">Nextcloud</a></li>
      <li><a href="/tutorials/linux/samba.php">Samba</a></li>
      <li><a href="/tutorials/linux/ssl.php">SSL</a></li>
      <li><a href="/tutorials/linux/users_groups.php">Users and groups</a></li>
    </ul>
    <li>IoT</li>
    <ul>
      <li><a href="/tutorials/iot/esp8266.php">ESP8266</a></li>
      <li><a href="/tutorials/iot/mqtt.php">MQTT</a></li>
      <li><a href="/tutorials/iot/sonoff.php">SONOFF</a></li>
      <li><a href="/tutorials/iot/sonoff_b1.php">SONOFF B1</a></li>
    </ul>
    <li>Electronics</li>
    <!--
    <ul>
        <li><a href="/tutorials/batteryless_smartphone.php">Batteryless smartphone</a></li>
    </ul>
    -->
    <li>Machine learning</li>
    <!--
    <ul>
        <li><a href="/tutorials/ml/intro.php">Introduction to machine learning</a></li>
        <li><a href="/tutorials/ml/neural_network.php">Artificial neural networks</a></li>
        <li><a href="/tutorials/ml/evolution_strategies.php">Evolution strategies</a></li>
    </ul>
    -->
    <li>Programming</li>
    <ul>
        <li>Python</li>
        <ul>
            <li><a href="/tutorials/programming/python/inheritance.php">Inheritance</a></li>
        </ul>
    </ul>
    <li>Web development</li>
    <ul>
        <li><a href="/tutorials/web/intro.php">Introduction to web development</a></li>
        <li>Client side</li>
        <ul>
            <li><a href="/tutorials/web/html.php">HTML</a></li>
            <li><a href="/tutorials/web/css.php">CSS</a></li>
            <ul>
                <li><a href="/tutorials/web/css/flex_distribute_elements_evenly.php">Distribute elements evenly with flex</a></li>
                <li><a href="/tutorials/web/css/grid_automatic_number_of_columns.php">Automatic number of columns with CSS grid</a></li>
            </ul>
            <li>JavaScript (coming soon)</li>
        </ul>
        <li>Server side</li>
        <ul>
            <li>PHP</li>
            <ul>
                <li>Introduction to PHP (coming soon)</li>
                <li><a href="/tutorials/web/sessions.php">Sessions in PHP</a></li>
            </ul>
            <li>Node.js</li>
            <ul>
                <li>Introduction to Node.js (coming soon)</li>
            </ul>
        </ul>
        <li>Misc</li>
        <ul>
            <li><a href="/tutorials/web/http_requests.php">HTTP requests</a></li>
        </ul>
    </ul>

</ul>

 <?php include $_SERVER["DOCUMENT_ROOT"].'/includes/post_main.php'; ?>
